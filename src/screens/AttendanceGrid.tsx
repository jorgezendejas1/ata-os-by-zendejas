...
export const getOperativeWeekForDate = (dateStr: string) => {
        ...
        if (terminal.hasZones) {
            effectiveCompanies.forEach(c => {
                terminalZones.forEach(z => {
                    cols.push({ id: `${c.id}_${z.id}`, companyId: c.id, companyName: c.name, zoneId: z.id, zoneName: z.name });
                });
            });
        } else {
            effectiveCompanies.forEach(c => {
                cols.push({ id: c.id, companyId: c.id, companyName: c.name, zoneId: undefined, zoneName: undefined });
            });
        }
        return cols;
...
// --- COMPONENTE PRINCIPAL: FORMULARIO DE CAPTURA (REGISTRO) ---

interface AttendanceGridProps {
  terminals: Terminal[];
  user: User;
  onSubmitRecords: (records: AttendanceRecord[]) => Promise<void>;
}
...
const AttendanceGrid: React.FC<AttendanceGridProps> = ({ terminals, user, onSubmitRecords }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSubmitRecords(recordsToSave);
